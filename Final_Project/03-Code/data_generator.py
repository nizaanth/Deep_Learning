...
class DataGenerator(keras.utils.Sequence):
    ...
    def __get_label(self, label):
        """Function to return the multi-hot encoded label.

        Args:
            label (np.array): multi-hot encoded label array

        Returns:
            np.array: array with shape [batch_size, n_classes]
        """
        # Since the labels are already multi-hot encoded,
        # just return the label as is.
        return label
